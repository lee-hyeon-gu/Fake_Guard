...
from dfadetect.models.gaussian_mixture_model import (GMMEM, GMMDescent,
                                                     flatten_dataset)
from dfadetect.models.SpecRNet import SpecRNet
from dfadetect.trainer import GDTrainer, GMMTrainer
from dfadetect.utils import set_seed
from experiment_config import get_specrnet_config, RAW_NET_CONFIG, feature_kwargs
...
def save_model(
        ...
        em: bool = False,
        raw_net: bool = False,
        specr_net: bool = False
) -> None:
    if raw_net:
        model_class = "raw_net"
    elif specr_net:
        model_class = "specr_net"
    else:
...
def train_specr_net(
    ...
    for current in fake_training_distributions:
        ...
        current_model = SpecRNet(specrnet_config, device=device)
        data_train = ConcatDataset([real_dataset_train, fake_dataset_train])
        LOGGER.info(
            f"Training SpecRNet model on {len(data_train)} audio files.")
        ...
        if model_dir is not None:
            save_model(current_model, model_dir, str(
                current).strip("/").replace("/", "_"), specr_net=True)
...
def main(args):
    ...
    if args.specr_net:
        init_logger("experiments_specrnet.log")
...
def parse_args():
    ...
    parser.add_argument(
        "--specr_net", help="Train specr net", action="store_true")
    parser.add_argument(
