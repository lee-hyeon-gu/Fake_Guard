...
if __name__ == '__main__':
    ...
    X_test = glob(f'{DATA_DIR}/*.jpg') #@@@@@ 확장자명 확인
    ...
    with torch.no_grad():
        for batch_index, (x, filename) in enumerate(tqdm(test_dataloader)):
            ...
            y_logits_j.append(y_logits)
            filenames.extend(filename)
    y_preds = torch.cat(y_preds, dim=0).tolist()
    y_logits_j = torch.cat(y_logits_j, dim=0).tolist()

    # Save predictions according to the sample submission format
    pred_df = pd.DataFrame({'ImageId':filenames, 'persent':y_logits_j, 'answer': y_preds})
